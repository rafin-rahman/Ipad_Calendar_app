...
class EventDAO{
    ...
    func getEvents(eventStartDate:Date, eventEndDate:Date, allDayStatus:Bool){
        ...
        let eventEndingToday = eventReference.whereField("EndTime", isLessThan: eventEndDate).whereField("All-Day", isEqualTo: allDayStatus).whereField("DeleteStatus", isEqualTo: false)
        eventEndingToday.getDocuments(){
            (querySnapshot, err) in
            if let err = err {
                print("Error getting documents: \(err)")
